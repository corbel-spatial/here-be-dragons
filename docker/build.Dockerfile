...
RUN uv pip install --system pyproj shapely --no-binary :all:
...
RUN uv pip install --system fiona geoalchemy2 geopandas geopy mapclassify psycopg psycopg2 || echo "WARNING: Extra packages did not build"
RUN uv cache clean
